            ...
            if gangvagar is not None:
                ...
                if not match.empty:
                    ...
                    if model is not None and label_map is not None:
                        ...
                        st.write("ML input:", input_data)
                        st.write("ML prediction (numeric):", prediction_numeric)
                        st.write("ML prediction (label):", prediction)
...
# SID: SPARADE RUTTER
    ...
    if not user_routes:
        ...
        for i, rutt in enumerate(user_routes):
            ...
            with col2:
                if st.button("Ta bort", key=f"delete_route_{i}"):
