...
# --- Filnamn för persistent lagring ---
ROUTES_FILE = "sparade_rutter.csv"
FORUM_FILE = "forum.csv"
REPORTS_FILE = "rapporter.csv"
...
# SID: RUTTER
    ...
    if st.button("Beräkna rutt"):
        ...
        if not start_coords or not end_coords:
            st.error("❌ En eller båda adresserna kunde inte hittas. Kontrollera stavningen och försök igen.")
            st.stop()
        from geopy.distance import geodesic
        distance_km = geodesic((start_coords[1], start_coords[0]), (end_coords[1], end_coords[0])).km
        if distance_km > 6000:
            st.error(f"❌ Rutten är {distance_km:.0f} km lång, vilket överskrider tillåten gräns.")
            st.stop()
...
# SID: SPARADE RUTTER
    ...
    if os.path.exists(ROUTES_FILE):
        df_routes = pd.read_csv(ROUTES_FILE)
        user_routes = df_routes[df_routes["email"] == user_info["email"]]
        if not user_routes.empty:
            for i, r in user_routes.iterrows():
                st.write(f"Rutt {i + 1}: Från {r['start']} → Till {r['end']}")
                col1, col2 = st.columns([1, 1])
                with col1:
                    if st.button(f"Visa rutt {i + 1}"):
                        coords = json.loads(r['route_coords'])
                        start = coords[0]
                        end = coords[-1]
                        show_map_with_position(coords, start, end)
                with col2:
                    if st.button(f"Ta bort rutt {i + 1}"):
                        df_routes = df_routes.drop(r.name)
                        df_routes.to_csv(ROUTES_FILE, index=False)
...
# SID: FORUM
    ...
    if os.path.exists(forum_file):
        try:
            ...
            forum_df = pd.DataFrame(columns=["Namn", "Kommentar", "E-post"])
            forum_posts = []
    else:
        forum_df = pd.DataFrame(columns=["Namn", "Kommentar", "E-post"])
        forum_posts = []
    ...
    if not forum_posts:
        ...
        for i, post in enumerate(reversed(forum_posts)):
            actual_index = len(forum_posts) - 1 - i
...
# SID: RAPPORTERING
    ...
    # Ladda rapporter från fil
    if os.path.exists(REPORTS_FILE):
        reports = pd.read_csv(REPORTS_FILE).to_dict(orient="records")
    else:
        reports = []
    if st.button("Skicka rapport"):
        if address and obstacle_type and description:
            ...
            if coords:
                ...
                pd.DataFrame(reports).to_csv(REPORTS_FILE, index=False)
