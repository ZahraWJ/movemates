...
# SID: RUTTER
    ...
    if st.button("Beräkna rutt"):
        ...
        try:
            ...
            if not os.path.exists("data/gangvagar_majorna.geojson"):
                st.warning("Dataset saknas i 'data/gangvagar_majorna.geojson'")
                st.stop()

            gangvagar = load_geojson()
            route_buffer = route_line.buffer(0.00005)  # Mindre buffer för exaktare match
            match = gangvagar[gangvagar.geometry.intersects(route_buffer)]

            # --- Analys av gångvägar (alltid visas) ---
            ...
            if match.empty:
                ...
                # Beräkna hur stor andel av rutten som täcks av Majorna-Linné-data
                matched_length = sum(geom.length * 111000 for geom in match.geometry if geom)
                route_length = route_line.length * 111000  # hela rutten i meter
                coverage_percent = (matched_length / route_length) * 100 if route_length > 0 else 0
                if coverage_percent < 100:
                    st.info(
                        f"Analysen nedan gäller endast {coverage_percent:.1f}% av din rutt, där den passerar genom området Majorna-Linné."
                    )
                    # Lista var analysen gäller med platsnamn
                    from openrouteservice import convert
                    for i, geom in enumerate(match.geometry):
                        start_coord = geom.coords[0]
                        try:
                            # Använd ORS reverse geocoding
                            reverse = client.pelias_reverse(point=[start_coord[0], start_coord[1]], size=1)
                            plats = reverse['features'][0]['properties'].get('label', 'Okänd plats')
                        except Exception:
                            plats = 'Okänd plats'
                        st.write(f"Analys-segment {i+1}: start vid {plats}")
                ...
                # Beräkna riskavsnitt
                risk_length = 0
                total_length = route_line.length * 111000  # Konvertera till meter (ungefärligt)
                for idx, row in match.iterrows():
                    if row.get('surface') in ['grus', 'kullersten', 'jord']:
                        risk_length += row.geometry.length * 111000  # Konvertera till meter
                ...
                # 🤖 ML-analys (endast för Majorna-Linné)
...
# --- LÄGG TILL CACHING DIREKT EFTER IMPORTS ---
@st.cache_data
def load_geojson():
    import geopandas as gpd
...
def load_ml_model():
    import joblib
    return joblib.load("ml_modell.pkl")
